...
public class BaseDao<E> implements Dao<E> {
	...
	public List<Object[]> doExecuteSql(String sql)
	{
		return getSession().createSQLQuery(sql).list();
	}
	
	@SuppressWarnings("unchecked")
	public int doInsertUpdateSql(String sql)
	{
	    ...
	    try{
	            //session = getSession();
	            number = getSession().createSQLQuery(sql).executeUpdate();
	    ...
	    {
	    	log.error("doInsertUpdateSql has error: ", e);
	    } finally
	    {
	    	try
	    	{
	    	...
	    	{
	    		log.error("doInsertUpdateSql close has error: ", e);
	    	}
