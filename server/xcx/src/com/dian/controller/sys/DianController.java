...
public class DianController extends DianBaseController<DianVue> {

	private static final Logger log = Logger.getLogger(DianController.class);
	...
	public void getForestry(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		QueryResult<DianVue> queryResult = dianService.doPaginationQuery(forestry);
		List<DianVue> forestryList = dianService.getForestryList(queryResult.getResultList());
		ListView<DianVue> forestryListView = new ListView<DianVue>();
	...
	public void deleteForestry(HttpServletRequest request, HttpServletResponse response, @RequestParam("ids") Long[] ids) throws IOException {
		boolean flag = dianService.deleteByPK(ids);
		if (flag) {
	...
	public void getForestryTypeName(HttpServletRequest request, HttpServletResponse response) throws Exception {
		List<DianType> forestryTypeList = dianTypeService.doQueryAll();
	...
	public void doSave(DianVue entity, HttpServletRequest request, HttpServletResponse response) throws IOException {
		ExtJSBaseParameter parameter = ((ExtJSBaseParameter) entity);
		DianVue checkEpcId = dianService.getByProerties("epcId", entity.getEpcId());
		if (null != checkEpcId && null == entity.getId()) {
			parameter.setSuccess(false);
		} else {
			DianType forestryType = dianTypeService.get(entity.getForestryTypeId());
			entity.setForestryType(forestryType);
			if (CMD_EDIT.equals(parameter.getCmd())) {
				dianService.update(entity);
			} else if (CMD_NEW.equals(parameter.getCmd())) {
				dianService.persist(entity);
			}
		...
		if (!file.isEmpty()) {
			if (file.getSize() > 2097152) {
				...
				try {
					...
					List<DianVue> list = objectToForestry(models);// Object-->Forestry
					for (int i = 0; i < list.size(); i++) {
						...
						DianVue checkForestryEpcId = dianService.getByProerties("epcId", list.get(i).getEpcId());
						...
						dianService.persist(list.get(i));
						count++;
					}
	...
	private List<DianVue> objectToForestry(List<Object[]> models) {
		List<DianVue> forestryList = new ArrayList<DianVue>();
		DianVue forestry = null;
		for (int i = 0; i < models.size(); i++) {
			try {
				forestry = new DianVue();
				...
				DianType forestryType = dianTypeService.getByProerties("name", models.get(i)[4].toString());
	...
	public void exportForestry(HttpServletRequest request, HttpServletResponse response, @RequestParam("ids") Long[] ids) throws Exception {
		List<Object[]> forestryList = dianService.queryExportedForestry(ids);
		//创建一个新的Excel
